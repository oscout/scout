...
const surface: TerminalSurface = {
  backend: "tmux",
  sessionName: "relay-openscout-main-dev-mac-mini-local-claude",
  paneId: null,
  attachCommand: ["tmux", "attach", "-t", "relay-openscout-main-dev-mac-mini-local-claude"],
  observeCommand: null,
  relay: { backend: "tmux", sessionName: "relay-openscout-main-dev-mac-mini-local-claude" },
};
...
describe("terminalSurfaceIdForSurface", () => {
  ...
  test("a pane-scoped handle does not match a session-scoped surface", () => {
    ...
    expect(terminalSurfaceMatchesId({ ...surface, paneId: "%3" }, "tmux:relay-openscout-main-dev-mac-mini-local-claude")).toBe(true);
  });
