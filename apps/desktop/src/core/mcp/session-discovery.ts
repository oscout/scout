...
import { SQLiteKnowledgeStore, type KnowledgeCoverage, type KnowledgeSearchHit } from "@openscout/runtime";
...
export function registerSessionDiscoveryTools(server: McpServer, deps: SessionDiscoveryDependencies): void {
  ...
  server.registerTool("sessions_search", {
    ...
    try {
      const coverage = summarizeCoverage(store.assessCoverage({ source: "sessions", harness, lookbackMs: hours * 3_600_000 }));
      const hits = coverage.kind === "warmed" ? store.searchLexical({ q: query, sourceKinds: ["sessions"], facets: { ...(harness ? { harness } : {}), ...(project ? { project } : {}) }, sourceUpdatedAfterMs: Date.now() - hours * 3_600_000, limit }) : [];
      ...
      const structuredContent = { query, mode: "lexical", coverage, indexedOnly: true, inventoryStatus: !hits.length ? "not_requested" : inventory ? "available_bounded" : "unavailable", results };
      return { content: [{ type: "text" as const, text: JSON.stringify(structuredContent) }], structuredContent };
    } finally { store.close(); }
  });
}
